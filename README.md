...
- **HRTF 角度匹配**: FreeSurround 底层算法生成的虚拟声场扬声器角度（如前置 ±27°）与 HeSuVi 等常见虚拟 7.1 HRTF 标准配置（如 ±30°）存在差异。为了获得最佳的声像定位，你需要提供与这些特殊角度相匹配的定制 HRTF 文件。
...
- **HRTF Angle Matching**: The virtual soundstage speaker angles generated by the underlying FreeSurround algorithm (e.g., front ±27°) differ from common standard virtual 7.1 HRTF configurations like HeSuVi (e.g., ±30°). To achieve optimal sound localization, you need to provide custom HRTF files that match these specific angles.
