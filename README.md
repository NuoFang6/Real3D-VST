...
Real3D-VST is a specialized audio plugin designed for multi-channel surround sound upmixing. Leveraging the classic FreeSurround decoding algorithm (foo_dsp_fsurround_source-0.9.0), it aims to expand traditional stereo (2.0) audio into a more immersive multi-channel (e.g., 5.1 or 7.1) experience in real-time. This project includes both a cross-platform VST version (compatible with Equalizer APO and HeSuVi) and a dedicated component for foobar2000.
...
- **HRTF Angle Matching**: The default virtual 7.1 soundstage speaker angles generated by the underlying FreeSurround algorithm (Front ±27°, Side ±95°, Rear ±142°) differ slightly from common standard virtual 7.1 HRTF configurations like HeSuVi (Front ±30°, Side ±90°, Rear ±135°).
...
- **16.1 Output Order (Fixed)**: When `Output Configuration` is set to `16.1 Surround`, the plugin outputs channels in FreeSurround native order without Windows-rank reordering, and keeps LFE at the end. Order: `FL, FCL, FC, FCR, FR, SFL, SFR, SCL, SCR, SBL, SBR, BL, BCL, BC, BCR, BR, LFE` (mapped to `out0..out16`).
